...
AudioStreamContext::AudioStreamContext(const vsdk::audio_info& audio_info, int buffer_duration_seconds)
    ...
    if (audio_info.sample_rate_hz <= 0) {
        VIAM_SDK_LOG(error) << "[AudioStreamContext] sample_rate_hz must be positive, got: " << audio_info.sample_rate_hz;
        throw std::invalid_argument("sample_rate_hz must be positive");
    }
    if (audio_info.num_channels <= 0) {
        VIAM_SDK_LOG(error) << "[AudioStreamContext] num_channels must be positive, got: " << audio_info.num_channels;
        throw std::invalid_argument("num_channels must be positive");
    }
    if (buffer_duration_seconds <= 0) {
        VIAM_SDK_LOG(error) << "[AudioStreamContext] buffer_duration_seconds must be positive, got: " << buffer_duration_seconds;
    ...
    if (buffer_capacity <= 0) {
        VIAM_SDK_LOG(error) << "[AudioStreamContext] buffer_capacity must be positive, calculated: " << buffer_capacity;
    ...
    try {
        ...
        VIAM_SDK_LOG(error) << "[AudioStreamContext] Failed to allocate audio buffer of size " << buffer_capacity
                            << " samples: " << e.what();
...
std::chrono::nanoseconds AudioStreamContext::calculate_sample_timestamp(uint64_t sample_number) noexcept {
...
std::chrono::nanoseconds calculate_sample_timestamp(const AudioStreamContext& ctx, uint64_t sample_number) {
    // Convert sample_number to frame number (samples include all channels)
