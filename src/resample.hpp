...
inline void convert_channels(const int16_t* input_samples,
    ...
    if (input_channels == 1 && output_channels == 2) {
        ...
        for (size_t i = 0; i < input_sample_count / 2; i++) {
            output_samples[i] = static_cast<int16_t>(
                (static_cast<int32_t>(input_samples[i * 2]) +
                 static_cast<int32_t>(input_samples[i * 2 + 1])) / 2);
        }
    } else {
        throw std::invalid_argument("Unsupported channel conversion from " + std::to_string(input_channels) +
                                    " to " + std::to_string(output_channels) + " channels");
    }
}
